  ...
  return produce(state, draft => {
    switch (action.type) {
      ...
      case LOAD_COUNTRIES_INFO_SUCCESS: {
        ...
        draft.list = newData.slice(draft.loadedItems.length, 30);
        draft.loadedItems = newData;
        break;
      }
      ...
      case SORT_BY_COLUMN: {
        ...
        draft.list = draft.loadedItems.slice(0, draft.loadSize);
        break;
      }
      case LOAD_MORE_ITEMS: {
        draft.list = draft.list.concat(draft.loadedItems.slice(draft.list.length, draft.list.length + draft.loadSize));
        break;
      }
      default: {
        break;
